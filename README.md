...
**Gen**, the mascot, is Gengo's own speech-bubble character (the logo brought to life, with the 言 mark on its corner). It idles with a gentle float and blinks, and reacts to what you do: jumps when you save a word, wobbles with raised brows on a wrong answer, spins and cheers when you hit the daily goal or finish a sprint, and looks thoughtful while a lookup is running. Its speech bubble is decorative for assistive technology; the same information reaches screen readers through the hidden announcer and toasts.
