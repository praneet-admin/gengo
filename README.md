...
| LibreTranslate | `POST {server}/translate` — default server `https://libretranslate.com` | the public server requires a key; self-hosted servers often don't |
| MyMemory (translation fallback) | `https://api.mymemory.translated.net/get?q={text}&langpair=en\|{lang}` | none (≈5,000 chars/day anonymous quota) |

**Translation setup.** Translation works out of the box through MyMemory. To use LibreTranslate instead, open **Translate → API settings** and enter a server URL and, if the server needs one, an API key (get one at portal.libretranslate.com, or run your own server). The URL is validated before any request. The key is held **in memory only** by default and forgotten when the tab closes. If you tick *Remember the key on this device* it is written to `localStorage`, which is **not** a secure store — only remember a key you could revoke, never a production secret. No key is included in this repository. If both LibreTranslate and MyMemory fail, a setup notice appears only inside the Translate section; everything else keeps working.
...
**Gen**, the mascot and logo, is Gengo's own purple bunny hugging a carrot that reads "Gengo" — drawn as an inline SVG so it scales from the favicon to the round-complete screen and can be animated. It breathes and blinks while idle, twitches an ear now and then, its pupils follow the pointer, and it reacts to what you do: hops when you save a word, droops its ears with a frown on a wrong answer, jumps and waves the carrot (squinting with a big grin) when you hit the daily goal or finish a sprint, and cocks an ear while a lookup is running. Small bunnies and carrots drift in the background and mark the empty states. The speech bubble is decorative for assistive technology; the same information reaches screen readers through the hidden announcer and toasts.
